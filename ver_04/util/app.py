...
class DataAllocate:
    ...
    class STLF:
        ...
        def allocator(load_vector, weather_vector, series_len, filter_type):
            ...
            if filter_type is DAY:
                train_input_vector = DataAllocate.STLF.filter_d(train_load, train_weather, series_len)
                test_input_vector = DataAllocate.STLF.filter_d(test_load, test_weather, series_len)
                return train_input_vector, test_input_vector
            elif filter_type is WEEK:
                train_input_vector = DataAllocate.STLF.filter_w(train_load, train_weather, series_len)
                test_input_vector = DataAllocate.STLF.filter_w(test_load, test_weather, series_len)
                return train_input_vector, test_input_vector
            elif filter_type is DAY_WEEK:
                train_input_vector = DataAllocate.STLF.filter_dw(train_load, train_weather, series_len)
                test_input_vector = DataAllocate.STLF.filter_dw(test_load, test_weather, series_len)
                return train_input_vector, test_input_vector
                ...
                for row in xrange(start_row, len(load_vector)):
                    for idx in xrange(0, series_len):
                        if idx == 0:
                            ...
                            weather_temp_w = DataAllocate.data_concatenate(weather_temp_w,
                                                                           weather_vector[row - 7 * idx])
    ...
    class MTLF:
        ...
        @staticmethod
        def combine_week_vector(data_vector):
            combined_vector = []
            for row in xrange(0, len(data_vector) / 7):
                combined_vector.append(data_vector[row * 7:(row + 1) * 7].flatten())
            return np.array(combined_vector)

        @staticmethod
        def allocator(load_vector, weather_vector):
            load_vector = DataAllocate.MTLF.combine_week_vector(load_vector)
            weather_vector = DataAllocate.MTLF.combine_week_vector(weather_vector)
            train_load, test_load = Preprocess.data_splitter(load_vector)
            train_weather, test_weather = Preprocess.data_splitter(weather_vector)
            train_input_vector = DataAllocate.MTLF.filter(train_load, train_weather)
            test_input_vector = DataAllocate.MTLF.filter(test_load, test_weather)
            return train_input_vector, test_input_vector

        @staticmethod
        def filter(load_vector, weather_vector):
            if len(load_vector) == len(weather_vector):
                input_vector = list()
                for row in xrange(0, len(load_vector)):
                    input_vector.append(DataAllocate.data_concatenate(load_vector[row], weather_vector[row]))
                return np.array(input_vector)
            else:
...
class SaveData:
    ...
    class STLF:
        ...
        def elm_based(input_vector, area_name, data_type, filter_type, series_len):
            save_path = '/Users/JH/Documents/GitHub/PowerForecast/ver_03/data/input_vector/STLF'
            ...
            for file_idx in xrange(0, len(selected_list) - 1):
                ...
                for filter_idx in xrange(0, len(filter_list)):
                    for series_len in xrange(1, 8):
                        train_input_vector, test_input_vector = DataAllocate.STLF.allocator(load_dict[area],
                                                                                            weather_vector,
                                                                                            series_len,
    ...
    class MTLF:
        ...
        @staticmethod
        def elm_based(input_vector, area_name, data_type):
            save_path = '/Users/JH/Documents/GitHub/PowerForecast/ver_04/data/input_vector/MTLF'
            save_path = os.path.join(save_path, area_name)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
        ...
        def input_vector(load_dict, weather_vector):
            ...
            for file_idx in xrange(0, len(area_names)):
                ...
                train_input_vector, test_input_vector = DataAllocate.MTLF.allocator(load_dict[area], weather_vector)
                print ' ',
                print SaveData.MTLF.elm_based(train_input_vector, area, 'train')
                print ' ',
                print SaveData.MTLF.elm_based(test_input_vector, area, 'test')
