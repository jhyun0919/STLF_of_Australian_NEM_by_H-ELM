...
logs_path = './tensorflow_logs/single_preprocessed'
test_result_directory = './single_preprocessed_result.bin'
...
def run_graph(data_set):
    ...
    for step in range(tf.app.flags.FLAGS.num_steps):
        ...
        avg_cost = 0.
        total_batch = int(data_set.PreProcessed.Train.feature.shape[0] / tf.app.flags.FLAGS.batch_size)

        # set a offset
        offset = (step * tf.app.flags.FLAGS.batch_size) % (
            data_set.PreProcessed.Train.target.shape[0] - tf.app.flags.FLAGS.batch_size)
        # Generate a minibatch.
        batch_train_x = data_set.PreProcessed.Train.feature[offset:(offset + tf.app.flags.FLAGS.batch_size), :]
        batch_train_y = data_set.PreProcessed.Train.target[offset:(offset + tf.app.flags.FLAGS.batch_size), :]
